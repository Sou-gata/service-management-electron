...
async function generatePdfViaIpc(htmlContent: string, filename: string, res: Response): Promise<boolean> {
    return new Promise((resolve) => {
        if (!process.env.IPC_PDF_PORT) { resolve(false); return; }
...
async function calculateDailyReportData(dateStr: string) {
    ...
    return rows.map((row) => {
        ...
        if (row.new_parts) {
            try {
                const parts = typeof row.new_parts === "string" ? JSON.parse(row.new_parts) : row.new_parts;
...
async function calculateReportData(type: string, year: number) {
    ...
    if (type === "weekly") {
        ...
        for (const row of rows) {
            ...
            if (year === currentYear && mondayTime > currentMondayTime) continue;

            if (!periods[key]) {
                periods[key] = { period: label, period_key: key, mondayTime, intakes_count: 0, completed_count: 0, delivered_count: 0, labor_revenue: 0, parts_cost: 0, net_revenue: 0, solved_count: 0 };
            }
                ...
                if (row.new_parts) {
                    try {
                        ...
                        if (Array.isArray(parts)) {
                            for (const part of parts) {
                                const partCost = parseFloat(part.cost);
                                if (!isNaN(partCost)) periods[key].parts_cost += partCost;
                            }
                        }
                    } catch (e) {}
                }
                if (row.is_solved === 1 || row.is_solved === true) periods[key].solved_count++;
            }
        }

        const reportData = Object.values(periods).sort((a, b) => b.mondayTime - a.mondayTime);
        reportData.forEach((p) => { p.net_revenue = p.labor_revenue + p.parts_cost; });
        return reportData;
    } else {
        ...
        for (let i = maxMonthIdx; i >= 0; i--) {
            ...
            periods[key] = { period: `${months[i]} ${year}`, period_key: key, intakes_count: 0, completed_count: 0, delivered_count: 0, labor_revenue: 0, parts_cost: 0, net_revenue: 0, solved_count: 0 };
        }
        ...
        const reportData = Object.values(periods).sort((a, b) => b.period_key.localeCompare(a.period_key));
        reportData.forEach((p) => { p.net_revenue = p.labor_revenue + p.parts_cost; });
        return reportData;
    }
}
...
export const getReportStats = asyncHandler(async (req: Request, res: Response) => {
    ...
    if (type === "daily") {
        ...
        return new ApiResponse(200, data, "Daily report stats retrieved successfully").send(res);
    } else {
        const reportType = type === "weekly" ? "weekly" : "monthly";
        const year = parseInt(req.query.year as string) || new Date().getFullYear();
        const data = await calculateReportData(reportType, year);
        return new ApiResponse(200, data, "Report stats retrieved successfully").send(res);
    }
});
...
export const getAvailableYears = asyncHandler(async (req: Request, res: Response) => {
    ...
    return new ApiResponse(200, years, "Available report years retrieved successfully").send(res);
});
...
export const downloadReportPdf = asyncHandler(async (req: Request, res: Response) => {
    ...
    if (type === "daily") {
        dateStr = (req.query.date as string) || new Date().toISOString().split("T")[0];
    ...
    let totalIntakes = 0, totalCompleted = 0, totalDelivered = 0;
    let totalLaborRevenue = 0, totalPartsCost = 0, totalNetRevenue = 0, totalSolved = 0;

    if (type === "daily") {
        totalIntakes = data.length;
        totalCompleted = data.filter(r => r.status === "Completed").length;
        totalDelivered = data.filter(r => r.status === "Delivered").length;
        totalLaborRevenue = data.reduce((sum, r) => sum + r.labor_revenue, 0);
        totalPartsCost = data.reduce((sum, r) => sum + r.parts_cost, 0);
        totalNetRevenue = data.reduce((sum, r) => sum + r.net_revenue, 0);
        totalSolved = data.filter(r => r.status === "Delivered" && (r.is_solved === 1 || r.is_solved === true)).length;
    } else {
        data.forEach((p) => {
    ...
    const overallSolvedRate = totalDelivered > 0 ? Math.round((totalSolved / totalDelivered) * 100) : 0;
    ...
    try {
        htmlContent = await ejs.renderFile(templatePath, {
            type, year, dateStr, periods: data,
            totals: { totalIntakes, totalCompleted, totalDelivered, totalLaborRevenue, totalPartsCost, totalNetRevenue, overallSolvedRate },
            generatedAt,
        });
    } catch (err: any) {
        throw new ApiErrorResponse(500, null, "Failed to render template: " + err.message);
    }

    const filename = type === "daily" ? `daily_report_${dateStr}.pdf` : `${type}_report_${year}.pdf`;
