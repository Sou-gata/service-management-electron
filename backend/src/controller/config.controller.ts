...
export const getDeviceTypes = asyncHandler(
    async (req: Request, res: Response) => {
        const [rows] = (await pool.query(
            "SELECT * FROM device_types ORDER BY name ASC"
        )) as [any[], any];
...
export const createDeviceType = asyncHandler(
    async (req: Request, res: Response) => {
        const { name } = req.body;
        if (!name || !name.trim()) {
        ...
        const [existing] = (await pool.query(
            "SELECT * FROM device_types WHERE LOWER(name) = ?",
...
export const updateDeviceType = asyncHandler(
    async (req: Request, res: Response) => {
        const { id } = req.params;
        const { name } = req.body;
        if (!name || !name.trim()) {
        ...
        const [existing] = (await pool.query(
            "SELECT * FROM device_types WHERE LOWER(name) = ? AND id != ?",
            [cleanName.toLowerCase(), id]
        )) as [any[], any];
        if (existing.length > 0) {
            throw new ApiErrorResponse(400, null, "Device type already exists");
        }
        ...
        return new ApiResponse(
            200,
            { id: Number(id), name: cleanName },
            "Device type updated successfully"
        ).send(res);
    }
);

export const deleteDeviceType = asyncHandler(
    async (req: Request, res: Response) => {
        const { id } = req.params;
        const [result] = (await pool.query(
...
export const getAccessories = asyncHandler(
    async (req: Request, res: Response) => {
        const [rows] = (await pool.query(
            "SELECT * FROM accessories ORDER BY name ASC"
        )) as [any[], any];
        return new ApiResponse(
            200,
            rows,
...
export const createAccessory = asyncHandler(
    async (req: Request, res: Response) => {
        ...
        const [existing] = (await pool.query(
            "SELECT * FROM accessories WHERE LOWER(name) = ?",
            [cleanName.toLowerCase()]
        )) as [any[], any];
        if (existing.length > 0) {
...
export const updateAccessory = asyncHandler(
    async (req: Request, res: Response) => {
        ...
        const [existing] = (await pool.query(
            "SELECT * FROM accessories WHERE LOWER(name) = ? AND id != ?",
            [cleanName.toLowerCase(), id]
        )) as [any[], any];
        if (existing.length > 0) {
            throw new ApiErrorResponse(400, null, "Accessory already exists");
