...
const HomeLayout = () => {
    ...
    const handleChangePassword = async (e) => {
        ...
        if (newPassword.length < 6) {
            toaster("error", "New password must be at least 6 characters.");
            return;
        }
        ...
        try {
            ...
            await apiService.put("/api/v1/users/me/password", {
                currentPassword,
                newPassword,
            });
    ...
    return (
        <div className="min-h-screen bg-background text-foreground flex font-sans overflow-hidden transition-colors duration-300">
                    ...
                    {!isCollapsed ? (
                        <div className="flex items-center justify-between p-2 rounded-lg bg-accent/40 border border-border/60 animate-in fade-in duration-200">
                            <div className="flex items-center gap-3">
                                <div
                                    className={`h-8 w-8 rounded-full font-bold text-xs flex items-center justify-center shadow-md shrink-0 ${
                                        user?.role === "admin"
                                            ? "bg-linear-to-br from-rose-500 to-amber-500 text-white shadow-rose-500/20"
                            ...
                            <div className="flex items-center gap-1 shrink-0">
                                <button
                                    ...
                                    className="p-1 rounded-md text-muted-foreground hover:text-amber-500 hover:bg-amber-500/10 transition-all cursor-pointer focus:outline-hidden"
            ...
            >
                <DialogContent className="sm:max-w-md border border-border bg-card/95 backdrop-blur-xl shadow-2xl rounded-2xl">
                    <DialogHeader>
                        ...
                        <DialogDescription className="text-xs text-muted-foreground">
                            Update your security credentials. Enter your current
                            password and a new secure password.
                        </DialogDescription>
                    </DialogHeader>
                    <form
                    ...
                    >
                        ...
                        <DialogFooter className="flex gap-2 justify-end pt-2">
                            ...
                            <button
                                type="submit"
                                disabled={isChanging}
                                className="flex items-center gap-1.5 px-4 py-2 bg-amber-600 hover:bg-amber-500 text-white rounded-lg text-xs font-semibold cursor-pointer transition-all border border-amber-500 shadow-sm"
                            >
                                {isChanging && (
                                    <svg
